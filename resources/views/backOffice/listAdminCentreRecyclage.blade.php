...
@section('dashboard-content')
    ...
    <div class="page-content-wrapper p-xxl-4">
        ...
        <div class="row">
            <div class="col-12 mb-4 mb-sm-5">
                <div class="d-sm-flex justify-content-between align-items-center">
                    <h1 class="h3 mb-3 mb-sm-0">Recycling Center Admin List</h1>
                    <div class="d-grid"><a data-bs-toggle="modal" data-bs-target="#exampleModal" class="btn btn-primary-soft mb-0"><i class="bi bi-person-fill-add me-2"></i>Add Recycling Center Admin</a>	</div>
                </div>
            </div>
        </div>
        ...
        <!-- agent list START -->
        <div class="row g-4">
            @foreach ($adminCentreRecyclages as $adminCentreRecyclage)
                <!-- Card item -->
                <div class="col-md-6 col-lg-4 col-xxl-3">
                    <div class="card border h-100">
                        ...
                        <div class="card-body text-center pb-0">
                            ...
                            <!-- Title -->
                            <h5 class="mb-1">{{ $adminCentreRecyclage->name }}</h5>
                            <small><i class="bi bi-person-badge me-1"></i>{{ $adminCentreRecyclage->matricule }}</small>
                            <!-- Info and rating -->
                            <div class="d-flex justify-content-between mt-3">
                                <h6 class="mb-0 small"><span class="fw-light">Email Address:</span></h6>
                                <h6 class="mb-0 small">{{ $adminCentreRecyclage->email }}</h6>
                            </div>
                            <div class="d-flex justify-content-between mt-3">
                                <h6 class="mb-0 small"><span class="fw-light">Phone Number:</span></h6>
                                <h6 class="mb-0 small">{{ $adminCentreRecyclage->telephone }}</h6>
                            </div>
                        </div>
                        <!-- card footer -->
                        <div class="card-footer d-flex gap-3 align-items-center">
                            <a href="{{ route('backOffice.detailAdminCentreRecyclage', $adminCentreRecyclage->id) }}" class="btn btn-sm btn-primary-soft mb-0 w-100">View detail</a>

                            <!-- Toggle enable/disable buttons -->
                            @if($adminCentreRecyclage->enable)
                                <form action="{{ route('user.disable', $adminCentreRecyclage->id) }}" method="POST">
                                    @csrf
                            ...
                            @else
                                <form action="{{ route('user.enable', $adminCentreRecyclage->id) }}" method="POST">
                                    @csrf
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">Add Recycling Center Admin</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form class="text-start" method="POST" action="{{ route('backOffice.addAdminCentreRecyclage') }}">
                        @csrf
