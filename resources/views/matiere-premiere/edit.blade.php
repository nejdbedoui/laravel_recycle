    ...
    <div class="row">
                <div class="col-12">
                    <div class="card border">
                        ...
                        <div class="card-header border-bottom">
                            <h5 class="card-header-title">Modifier la matière Première</h5>
                        </div>
                        <!-- Card header END -->
                        <div class="card-body bg-light bordred rounded">
        @if ($errors->any())
        ...
        @endif
        <form action="{{ route('matiere-premiere.update', $matierePremiere->id) }}" method="POST">
    @csrf
    @method('PUT')

    <div class="form-group mb-3">
        <label for="nom">Nom:</label>
        <input type="text" name="nom" id="nom" class="form-control" value="{{ old('nom', $matierePremiere->nom) }}" required>
        @error('nom')
            <div class="text-danger">{{ $message }}</div> <!-- Error message -->
        @enderror
    </div>

    <div class="form-group mb-3">
        <label for="quantite">Quantité:</label>
        <input type="number" name="quantite" id="quantite" class="form-control" value="{{ old('quantite', $matierePremiere->quantite) }}" required min="1">
        @error('quantite')
            <div class="text-danger">{{ $message }}</div> <!-- Error message -->
        @enderror
    </div>
    ...
    <div class="form-group mb-3">
        ...
        <select name="centre_recyclage_id" id="centre_recyclage_id" class="form-control" required>
            ...
            @foreach ($centres as $centre)
                <option value="{{ $centre->id }}" {{ $centre->id == old('centre_recyclage_id', $matierePremiere->centre_recyclage_id) ? 'selected' : '' }}>
                    {{ $centre->nom }}
                </option>
            @endforeach
        </select>
    ...
    <div class="d-flex justify-content-between mt-4">
        ...
        <a href="{{ route('matiere-premiere.index') }}" class="btn btn-outline-secondary">Annuler</a>
    </div>
</form>
