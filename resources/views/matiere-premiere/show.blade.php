                ...
                <div class="col-12">
                    <div class="card border">
                        ...
                        <div class="card-header border-bottom">
                            <h5 class="card-header-title">La matière première : {{ $matierePremiere->nom }}</h5>
                        </div>
              <!-- Card Container -->
    <!-- Card Body -->
    <div class="card-body">
    <div class="d-flex flex-column align-items-start mb-3">
        <p class="h6"><strong>Quantité:</strong> <span class="text-primary fs-5">{{ $matierePremiere->quantite }}</span></p>
        <p class="h6"><strong>Centre de Recyclage:</strong> <span class="text-secondary fs-5">{{ $matierePremiere->centreRecyclage->nom }}</span></p>
        <p class="h6"><strong>Adresse du Centre de Recyclage:</strong> <span class="text-secondary fs-5">{{ $matierePremiere->centreRecyclage->adresse }}</span></p>
    </div>
    <hr>
    <p class="text-muted fs-6">Gestion des matières premières pour un recyclage efficace.</p>
</div>

    <!-- Card Footer -->
    <div class="card-footer d-flex justify-content-around">
        <a href="{{ route('matiere-premiere.edit', $matierePremiere->id) }}" class="btn btn-outline-warning">Modifier</a>
        <form action="{{ route('matiere-premiere.destroy', $matierePremiere->id) }}" method="POST" style="display:inline;">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-outline-danger" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette matière première ?');">Supprimer</button>
        </form>
        <a href="{{ route('matiere-premiere.index') }}" class="btn btn-outline-secondary">Retour</a>
    </div>
</div>
